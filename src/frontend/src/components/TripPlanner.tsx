...
function extractCoordinatesFromNotes(
  notes: string,
): { lat: number; lng: number } | null {
  const coordMatch = notes.match(
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 max-w-2xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-4 text-sm text-blue-800">
            ...
            <div className="flex items-start space-x-2">
              ...
              <span>
                View detailed trip information and suggested activities
              </span>
            </div>
            <div className="flex items-start space-x-2">
      ...
      <TripDetail
        ...
        onEdit={
          canEditTrip(selectedTrip)
            ? (tripData) => handleEditTrip(selectedTrip.id, tripData)
            : undefined
        }
        onDelete={
        ...
        {filteredTrips.length === 0 ? (
          ...
          filteredTrips.map((trip) => (
            <TripCard
              ...
              onEdit={
                canEditTrip(trip)
                  ? (tripData) => handleEditTrip(trip.id, tripData)
                  : undefined
              }
              onDelete={
                canDeleteTrip(trip)
      ...
      {showDeleteAllConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete all {trips.length} trips? This
              action cannot be undone and will permanently remove all trip data.
            </p>
      ...
      {showDeleteConfirm && onDelete && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete "{trip.name}"? This action cannot
              be undone.
            </p>
            <div className="flex justify-end space-x-3">
              <button
    ...
    const calculateTotalDistance = async () => {
      ...
      try {
        for (let i = 0; i < selectedPlaces.length - 1; i++) {
          ...
          const coords2 = extractCoordinatesFromNotes(
            selectedPlaces[i + 1].notes,
          );
            ...
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              {totalDistance > 0 && (
                <div className="mt-2 text-xs text-blue-700">
                  ...
                  Total travel distance: <strong>{totalDistance} km</strong>{" "}
                  (calculated using coordinates)
                </div>
              )}
            </div>
          )}
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto border border-gray-200 rounded-lg">
            {filteredPlaces.length === 0 ? (
              <div className="p-4 text-center text-gray-500">
                {places.length === 0
                  ? "No places available. Add some places first!"
  ...
  const suggestedActivities = useMemo(() => {
    ...
    for (const tag of uniqueTags) {
      ...
      if (tagActivities_list) {
        for (const activity of tagActivities_list) {
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border">
          <div className="p-6 border-b">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">
                Trip Itinerary ({trip.places.length} places)
              </h3>
              {isCalculatingDistances && (
                <div className="flex items-center space-x-2 text-sm text-gray-600">
                  <Loader className="w-4 h-4 animate-spin" />
              ...
              <div className="space-y-6">
                {placesWithDistances.map((place, index) => (
                  <div key={place.id} className="relative">
                    ...
                    {place.distanceToNext !== undefined &&
                      place.distanceToNext > 0 && (
                        <div className="ml-8 mt-2 mb-4">
                          <div className="flex items-center space-x-2 text-sm text-gray-600 bg-gray-50 px-3 py-2 rounded-lg">
                            <Navigation className="w-4 h-4" />
                            <span>
                              <strong>{place.distanceToNext} km</strong> to next
                              destination
                            </span>
                          </div>
                        </div>
        ...
        {suggestedActivities.length > 0 && (
          <div className="bg-white rounded-lg shadow-sm border">
            <div className="p-6 border-b">
              ...
              <p className="text-sm text-gray-600 mt-1">
                Based on the places in your trip
              </p>
            </div>
            <div className="p-6">
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-800">
                  Distances are calculated between consecutive places in your
                  itinerary using coordinate data when available. This helps you
                  plan travel time and optimize your route for efficiency.
                  {totalDistance === 0 && (
                    <span className="block mt-1 font-medium">
                      Note: Distance calculation requires coordinate data from
                      places with GPS information.
                    </span>
                  )}
                </p>
              </div>
