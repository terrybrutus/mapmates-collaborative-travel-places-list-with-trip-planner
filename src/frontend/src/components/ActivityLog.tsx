...
export default function ActivityLog() {
  ...
  const [filterType, setFilterType] = useState<
    "all" | "signup" | "place" | "trip" | "discovery"
  >("all");
  ...
  const users = useMemo(() => {
    const uniquePrincipals = Array.from(
      new Set(activities.map((a) => a.user.toString())),
    );
  ...
  const getTotalActivitiesCount = () => {
    return filteredGroupedActivities.reduce(
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-800">
                This activity log shows real user actions based on actual place
                additions and modifications. Activities are grouped by user and
                ordered by recency. User names with partial principal IDs are
                displayed for transparency and user identification.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900">
                {
                  activities.filter((a) =>
                    a.action.toLowerCase().includes("added place"),
                  ).length
                }
              </p>
            </div>
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border">
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900">
                {filteredGroupedActivities.length}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm">
        <div className="p-6 border-b">
          <h3 className="text-lg font-semibold text-gray-900">
            Recent Activity (Grouped by User)
          </h3>
          <p className="text-sm text-gray-600 mt-1">
            Real activity tracking based on actual user actions. Activities are
            grouped by user and ordered by most recent activity. Click to expand
            individual actions.
          </p>
        </div>
              ...
              {activities.length === 0 ? (
                <>
                  ...
                  <p className="text-gray-600">
                    Start adding places to see activity here. All user actions
                      ...
                      <div className="mt-2 ml-9 text-sm text-gray-600">
                        ...
                        {group.activities.length > 1 && (
                          <span className="ml-2 text-gray-500">
                            and {group.activities.length - 1} other activit
                            {group.activities.length - 1 === 1 ? "y" : "ies"}
                          </span>
                        )}
                      </div>
                    )}
                ...
                {expandedGroups.has(group.user) && (
                  <div className="bg-gray-50 border-t">
                    {group.activities.map((activity) => (
                      <div
                        key={`${activity.user.toString()}-${String(activity.timestamp)}`}
                        className="p-4 ml-4 border-l-2 border-gray-200"
                      >
                        <div className="flex items-start space-x-4">
                          <div
                            className={`p-2 rounded-lg border ${getActivityColor(activity.action)}`}
                          >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center space-x-4 text-xs text-gray-500">
                              ...
                              <span>
                                {new Date(
                                  Number(activity.timestamp) / 1000000,
                                ).toLocaleDateString()}
                              </span>
                            </div>
                          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2">
                {filteredGroupedActivities.slice(0, 5).map((group, _index) => (
                  <div
                    key={group.user}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-medium text-gray-900">
                    {
                      activities.filter((a) =>
                        a.action.toLowerCase().includes("place"),
                      ).length
                    }
                  </span>
                </div>
