...
function extractGeocodingData(notes: string): {
  timezone?: string;
  isOnWater?: boolean;
} {
  ...
  return {
    ...
    isOnWater: waterMatch
      ? ["yes", "true"].includes(waterMatch[1].toLowerCase())
      : undefined,
  };
}
...
export default function PlaceCard({
  ...
  // In custom auth, we can't easily compare principal, so allow admins and any authenticated user to edit
  const canEditPlace = !hideEditControls && isAuthenticated && isAdmin;
  const canDeletePlace =
    !hideEditControls && canEdit && isAuthenticated && isAdmin;
        ...
        <div className={compact ? "p-4" : "p-6"}>
          ...
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1 min-w-0">
              <h3
                className={`${compact ? "text-md" : "text-lg"} font-semibold text-gray-900 mb-1 break-words`}
              >
                {place.city}
                {place.stateRegion?.trim() && `, ${place.stateRegion}`}
                {!compact && `, ${place.country}`}
              </h3>
              {!compact && (
                <div className="flex items-center text-gray-600 mb-2">
                  <span className="text-lg mr-2">
                    {getCountryFlag(place.country)}
                  </span>
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    {place.attractions.length > (compact ? 2 : 3) && (
                      <span className="text-xs text-gray-500">
                        +{place.attractions.length - (compact ? 2 : 3)} more
                      </span>
                    )}
                  </div>
                </div>
              ...
              {(timezone || isOnWater !== undefined) && (
                <div className="mb-2 flex flex-wrap gap-2">
                  {timezone && (
                    <span
                      className={`inline-flex items-center px-2 py-1 rounded-full ${compact ? "text-xs" : "text-xs"} font-medium bg-purple-100 text-purple-800 border border-purple-200`}
                    >
                  ...
                  {isOnWater !== undefined && (
                    <span
                      className={`inline-flex items-center px-2 py-1 rounded-full ${compact ? "text-xs" : "text-xs"} font-medium ${isOnWater ? "bg-blue-100 text-blue-800 border-blue-200" : "bg-gray-100 text-gray-800 border-gray-200"} border`}
                    >
              ...
              <div className="flex items-center text-xs text-gray-500 mb-2">
                ...
                <span className="break-words">
                  {`Created by ${place.author.toString().slice(0, 8)}...`}
                </span>
              </div>
            </div>
          ...
          {place.status.length > 0 && (
            <div className="mb-3">
              <div className="flex flex-wrap gap-1">
                {place.status.slice(0, compact ? 2 : 6).map((status) => (
                  <span
                    key={status}
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center justify-between mt-2">
                <p className="text-xs text-gray-500">
                  Cover photo • {place.images.length} image
                  {place.images.length !== 1 ? "s" : ""} total
                </p>
                <button
                  type="button"
                  onClick={() => openLightbox(0)}
          ...
          {place.quickFacts.bestKnownFor && (
            <div className="mb-3">
              <p
                className={`${compact ? "text-xs" : "text-sm"} text-gray-600 break-words`}
              >
          ...
          {place.notes && (
            <div className="mb-3">
              <p
                className={`${compact ? "text-xs" : "text-sm"} text-gray-700 ${compact ? "line-clamp-2" : "line-clamp-3"} break-words`}
              >
                {place.notes}
              </p>
            </div>
          ...
          {formattedTags.length > 0 && (
            <div className="mb-3">
              <div className="flex flex-wrap gap-1">
                {formattedTags.slice(0, compact ? 2 : 3).map((tag) => (
                  <span
                    key={tag}
                ...
                {formattedTags.length > (compact ? 2 : 3) && (
                  <span className="text-xs text-gray-500">
                    +{formattedTags.length - (compact ? 2 : 3)} more
                  </span>
                )}
              </div>
            </div>
          ...
          {/* Footer */}
          <div
            className={`flex items-center justify-between ${compact ? "text-xs" : "text-sm"} text-gray-500 pt-3 border-t flex-wrap gap-2`}
          >
            ...
            <span
              className={`${compact ? "text-xs" : "text-xs"} flex-shrink-0`}
            >
          ...
          >
            ...
            >
              ...
              <p className="text-gray-600 mb-6 break-words">
                Are you sure you want to delete "{place.city}, {place.country}"?
...
function CoverPhotoThumbnail({
  ...
  return (
    <button
      type="button"
      className="relative w-full h-48 bg-gray-100 rounded-lg overflow-hidden cursor-pointer hover:opacity-90 transition-opacity group"
      onClick={onClick}
      aria-label={`View ${placeName}`}
    >
      <img
        src={imageUrl}
        alt={placeName}
...
function CompactPhotoThumbnail({
  ...
  return (
    <button
      type="button"
      className="relative w-full h-24 bg-gray-100 rounded-md overflow-hidden cursor-pointer hover:opacity-90 transition-opacity"
      onClick={onClick}
      aria-label={`View ${placeName}`}
    >
      <img
