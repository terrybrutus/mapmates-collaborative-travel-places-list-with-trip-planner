...
export default function ImportModal({ isOpen, onClose }: ImportModalProps) {
  ...
  const [step, setStep] = useState<
    "input" | "review" | "importing" | "complete"
  >("input");
  ...
  const handleFileSelect = (selectedFile: File) => {
    ...
    if (
      ...
      !["txt", "pdf"].includes(fileExtension || "")
    ) {
  ...
  const parseFTITContent = (content: string): ParsedPlace[] => {
    ...
    blocks.forEach((block, blockIndex) => {
      ...
      if (!hasRequiredFields || !place.country || !place.city) {
        place.isValid = false;
        place.errors.push(
          "Missing required fields: Country and City/Town/Village are mandatory",
        );
      }
  ...
  const processContent = async (content: string) => {
    ...
    try {
      ...
      const validPlaceIds = new Set(
        parsed.filter((p) => p.isValid).map((p) => p.id),
      );
  ...
  const handleImport = async () => {
    ...
    try {
      const placesToImport = parsedPlaces.filter(
        (p) => selectedPlaces.has(p.id) && p.isValid,
      );
  ...
  const selectAll = () => {
    const validPlaceIds = new Set(
      parsedPlaces.filter((p) => p.isValid).map((p) => p.id),
    );
    setSelectedPlaces(validPlaceIds);
  };
            ...
            <div className="space-y-6">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h3 className="font-medium text-blue-900 mb-2">
                  FTIT Format Requirements
                </h3>
                <div className="text-sm text-blue-800 space-y-1">
                  <p>
                    • Each place should be in a separate block (separated by
                    blank lines)
                  </p>
                  <p>
                    • Use{" "}
                  ...
                  <p>
                    • Required fields: <strong>Country</strong> and{" "}
                    <strong>City/Town/Village</strong>
                  </p>
                  <p>
                    • Optional fields: State/Region, Attractions, Notes, Tags,
                    Budget Range, Best Time to Visit, Best Known For, Hidden
                    Gem, Local Tip
                  </p>
          ...
          {step === "review" && (
            <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row justify-between space-y-2 sm:space-y-0 sm:space-x-3">
                ...
                >
                  Import {selectedPlaces.size} Place
                  {selectedPlaces.size !== 1 ? "s" : ""}
                </button>
              </div>
            </div>
          )}
          ...
          {step === "importing" && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">
                Please wait while we process your selected places.
              </p>
              <button
                type="button"
                onClick={() => {
