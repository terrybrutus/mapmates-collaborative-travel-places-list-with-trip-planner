...
// Stable utility — defined outside component to avoid re-creating on every render
...
export default function PlacesList() {
  ...
  const [sortBy, setSortBy] = useState<"name" | "country" | "timestamp">(
    "country",
  );
  const [showDropdown, setShowDropdown] = useState(false);
  const [activeView, setActiveView] = useState<"list" | "add" | "random">(
    "list",
  );
  ...
  const countries = useMemo(() => {
    const uniqueCountries = Array.from(
      new Set(places.map((p) => getStandardCountryName(p.country))),
    ).sort();
  ...
  const searchPlaces = useCallback(
    (places: Place[], searchTerm: string): Place[] => {
      ...
      const matchingCountries = new Set(
        countrySuggestions.map((c) => normalizeText(c)),
      );
  ...
  const filteredPlaces = useMemo(() => {
    ...
    if (statusFilter !== "all") {
      filtered = filtered.filter((place) =>
        place.status.includes(statusFilter),
      );
    }
    ...
    if (countryFilter !== "all") {
      ...
      filtered = filtered.filter(
        (place) => getStandardCountryName(place.country) === standardizedFilter,
      );
    }
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case "country":
          return getStandardCountryName(a.country).localeCompare(
            getStandardCountryName(b.country),
          );
        case "timestamp":
  ...
  const hierarchicalPlaces = useMemo(() => {
    ...
    for (const place of filteredPlaces) {
      ...
      cityKey = place.city?.trim() ? place.city.trim() : "Unknown City";

      // Use state/region if available, otherwise use city as region too
      if (place.stateRegion?.trim()) {
  ...
  const toggleCountry = (country: string) => {
    ...
    if (newExpanded.has(country)) {
      ...
      for (const region of Object.keys(
        hierarchicalPlaces[country]?.regions || {},
      )) {
  ...
  return (
    <div className="main-content-container">
      <div className="space-y-6">
        ...
        {activeView === "random" && <RandomPicker />}
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm">
              <div className="grid md:grid-cols-4 gap-4">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSortBy(
                      e.target.value as "name" | "country" | "timestamp",
                    )
                  }
                  className="px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
              ...
              {searchTerm.trim() && (
                <div className="mt-3 text-sm text-gray-600">
                  <p>
                    <strong>Enhanced Search:</strong> Search ignores punctuation
                    and is case-insensitive. You can search by attractions,
                    places, neighborhoods, countries (including abbreviations
                    like "USA", "UK", "JAP"), cities, regions, tags, activities,
                    notes, or any other place details. Multi-status places are
                    included if any status matches.
                  </p>
                </div>
              )}
            </div>
                ...
                {Object.entries(hierarchicalPlaces)
                  ...
                  .map(([country, countryData]) => (
                    <div key={country} className="hierarchical-country-group">
                      ...
                      <button
                        type="button"
                        className="hierarchical-country-header w-full text-left"
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="text-sm text-gray-500">
                            (
                            {Object.values(countryData.regions).reduce(
                              (total, region) =>
                                total +
                                Object.values(region.cities).reduce(
                                  (cityTotal, cityPlaces) =>
                                    cityTotal + cityPlaces.length,
                                  0,
                                ),
                              0,
                            )}{" "}
                        ...
                        <div className="hierarchical-country-content">
                          {Object.entries(countryData.regions)
                            ...
                            .map(([region, regionData]) => {
                              const regionKey = `${country}-${region}`;
                              const regionPlaceCount = Object.values(
                                regionData.cities,
                              ).reduce(
                                (total, cityPlaces) =>
                              ...
                              return (
                                <div
                                  key={regionKey}
                                  className="hierarchical-region-group"
                                >
                                  {/* Region Header */}
                                  <button
                                    type="button"
                                    className="hierarchical-region-header w-full text-left"
                                    onClick={() =>
                                    ...
                                    <div className="hierarchical-region-content">
                                      {Object.entries(regionData.cities)
                                        ...
                                        .map(([city, cityPlaces]) => (
                                          <div
                                            key={`${regionKey}-${city}`}
                                            className="hierarchical-city-group"
                                          >
